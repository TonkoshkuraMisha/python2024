...
# Частота выигрышей с округлением до 5 знаков
...
# Сортировка статистики по комбинациям от дешёвой до дорогой
sorted_combinations = sorted(paytable.keys(), key=lambda k: paytable[k])

# Результаты
print(f"Общий RTP: {game_rtp:.2f}%")
print(f"RTP Джекпота: {jackpot_rtp * 100:.2f}%")
print("Частоты выигрышей по комбинациям (от дешёвой до дорогой):")
for combination in sorted_combinations:
    print(f"Комбинация: {combination}, Частота: {win_frequencies.get(combination, '0.00000')}")

print("Распределение выигрышей по символам (от дешёвой до дорогой):")
for combination in sorted_combinations:
    print(f"Комбинация: {combination}, Выигрыш: {win_distribution.get(combination, 0)}")

print("Средние интервалы между выигрышами по комбинациям (от дешёвой до дорогой):")
for combination in sorted_combinations:
    print(f"Комбинация: {combination}, Средний интервал: {interval_means.get(combination, '0.00000')}")

print("Стандартные отклонения интервалов выигрышей по комбинациям (от дешёвой до дорогой):")
for combination in sorted_combinations:
    print(f"Комбинация: {combination}, Стандартное отклонение: {interval_stds.get(combination, '0.00000')}")
